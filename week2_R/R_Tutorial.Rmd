...
```{r setup}
    knitr::opts_knit$set(root.dir = normalizePath("/Users/amirarya/Desktop/qbio_490_amir/week2_R")) 
```
...
```{r}
sum(na_boolean_mask)
amountOfTrueValues <- sum(na_boolean_mask)
```
...
```{r}
mask <- c(TRUE, TRUE, FALSE, FALSE, TRUE) #c stands for combined 
...
THINK: When using is.na(), are NA values assigned TRUE or FALSE? NA VALUES ARE ASSIGNED TRUE With this information in mind, what does TRUE mean in your 'na_boolean_mask' vector? Is that data we want to keep, or data we want to ignore. DATA WE WANT
...
```{r}
meanPeatalLength <- mean(iris$Petal.Length)
meanPeatalWidth <- mean(iris$Petal.Width) 

color <- ifelse(iris$Petal.Length > meanPeatalLength & iris$Petal.Width > meanPeatalWidth, "red" ,
                ifelse(iris$Petal.Length > meanPeatalLength & iris$Petal.Width < meanPeatalWidth,  "blue" ,
                       ifelse(iris$Petal.Length < meanPeatalLength & iris$Petal.Width < meanPeatalWidth, "green",
                          ifelse(iris$Petal.Length < meanPeatalLength & iris$Petal.Width > meanPeatalWidth, "yellow", "black"))))

as.factor(color)

print(cor.test(iris$Petal.Length, iris$Petal.Width))

plot(iris$Sepal.Length,iris$Petal.Width, col = color)
abline(h = meanPeatalWidth, v = meanPeatalLength)
...
```{r}
write.csv(attenu_cleaned, "/Users/amirarya/Desktop/attenu_cleaned.csv")
```
...
```{r}
attenu_read_in <- read.csv ("/Users/amirarya/Desktop/attenu_cleaned.csv", header = TRUE)
```
...
```{r}
pdf("/Users/amirarya/Desktop/attenu_cleaned.pdf")
